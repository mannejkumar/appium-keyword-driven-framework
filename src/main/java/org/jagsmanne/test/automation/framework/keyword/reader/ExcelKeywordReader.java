...
import org.jagsmanne.test.automation.framework.util.readers.ExcelReader;
...
public class ExcelKeywordReader implements IKeywordReader {
	...
	private List<IKeywordStore> readData(ExcelReader excelReader) {
		...
		for (int rowNo = 1; rowNo < noOfRows; rowNo++) {
			...
			String locator = excelReader.getData(rowNo, 1);
			String locatorType = excelReader.getData(rowNo, 2);
			List<Object> valueList = new ArrayList<Object>();

			for (int columnNo = 3; columnNo < excelReader.getNoOfColumn(rowNo); columnNo++) {
